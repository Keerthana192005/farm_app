...
def admin_reports():
    try:
        ...
        total_revenue = sum(order.total for order in orders)
        total_customers = len({(order.customer_name, order.phone, order.email) for order in orders})
    except Exception as e:
        print(f"Database error: {e}")
        orders = []
        total_orders = 0
...
def admin_customers():
    try:
        ...
        for order in orders:
            ...
            if order.date > customers[order.customer_name]['last_order']:
                customers[order.customer_name]['last_order'] = order.date
...
# Vercel/PythonAnywhere entrypoint
application = app
